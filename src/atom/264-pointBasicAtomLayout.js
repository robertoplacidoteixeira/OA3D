class AtomPointsSceneryLayout extends AtomBasicSceneryLayout {
	...
	createPointsMaterial(color_,opacity) {
	   return new THREE.PointsMaterial({size: this.pointSize, color: color_, opacity: opacity, sizeAttenuation: true, vertexColors: THREE.VertexColors});
	}
	...
	createPointsMesh(geo_,mat_) {
	   return new THREE.Points(geo_,mat_);
	}
