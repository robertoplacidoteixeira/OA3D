class Schrodinger extends RptCommonAncestor{
	...
	afterPrepare() {
		...
		this.swf = new function() {
			...
			var a0    = 4 * pi * eps * ((h_bar / q) ** 2) / mass;
			// var a0    = mul(complex(4),pi,eps,pow(h_bar/q,2)/mass;   //% Bohr radius
			/*
			// laguerre Calculates associated Laguerre Polynomial value
			laguerre = function(a,b,x) {
				var k = mt.add(a,b);
				for (var i = 1; i <= a; i++) {
					f = mt.diff(function(x){mt.pow(x,k)*mt.exp(-y)});
				}
				g = mt.div(mt.mul(pow(y,-b),mt.exp(y)),mt.factorial(a));
				L = vpa(subs(g * f,y,x));
			}
			...
			legendre(a,b,x) {
				...
				g = (-1)^b * pow(1-y**2,b/2) / (2^a * factorial(a));
				P = vpa(subs(g * f,y,x));
			}
			...
			eigen_H(eigenV) {
				...
				r = 0.5;
				//  k = q^2/(4 * pi * eps);
				cx = mt.div(mt.pow(h_bar,2),mt.mul(2,mass));
				d = 1;
				var model = this.createpde;
				// //% Potential function
			...
			inf_well_1d_02(Lz) { // Lz > 0
				...
				for (var nE = 1; nE < n.lenght; i++) {
					E_nE = mt.pow((h_bar * nE * pi/Lz),2) / (2 * mass);
					E(nE,:) = E_nE;
					plot(z,E(nE,:),'r-');
			...
			*/
			...
			this.atomHidrogen = function(scene,n,l,ml) {
				/*
				...
				*/
				...
				for (var nE = 1; nE <= n_max; nE++) {
					E_nE = mt.mul(mt.neg(mass),mt.div(mt.pow(mt.div(mt.pow(q,2),mt.mul(pi,eps,h_bar,nE)),2),32));
					E(nE,:) = E_nE;
					plot(z_E,E(nE,:),'r-');
				...
				*/
				...
				const r_max = 1.2e-1; // 120e-12;        //% Van der Vaal's radius of Hydrogen
				const dp = 15;

				const aux = r_max / dp;
				
				var geometry = THREE.Sphere(aux,16,16);
				...
				for(var x = -r_max; x <= r_max; x += aux) {
					for(var y = -r_max; y <= r_max; y += aux) {
						for(var z = -r_max; z <= r_max; z += aux) {
							...
							const psi = waveFunctionHidrogen(n,l,ml,r,theta,phi);
							const prob = mt.pow(abs(psi),2);
			...
			function radialWaveFunction(n,l,r) {
					var a = mt.sqrt(mt.div(mt.factorial(n-l-1),mt.mul(2,n,mt.pow(mt.factorial(mt.add(n,1)),3))));
					var b = mt.pow(mt.div(2,mt.mul(n,a0)),mt.div(mt.add(l,3),2));
					var c = mt.laguerre(mt.add(n,l),mt.add(mt.mul(2,l),1),mt.div(mt.mul(2,r),mt.mul(n,a0)));
					var d = mt.pow(R,l);
					var e = mt.exp(div(mt.neg(R),mt.mul(n,a0)));
					var f = mt.mul(a,b,c,d,e);
					return
					/*
					mt.mul(
						mt.sqrt(div(mt.factorial(n-l-1),mt.mul(2,n,mt.pow(mt.factorial(mt.add(n,1)),3)))),
						mt.pow(mt.div(2,mt.mul(n,a0)),mt.div(mt.add(l,3),2)), 
			...
			function sphericalHarmonics(signal,l,ml,theta,phi) {
				var aml = mt.abs(ml);
				return
					mt.mul(
						mt.sqrt(
			...
			function psi(rwf,l,ml,theta,phi) {
			...
			function waveFunctionHidrogen(n,l,ml,R,theta,phi) {
				return psi(radialWaveFunction(n,l,R),l,ml,theta,phi);
			}
		}();
	}
...
function iniciar() {
	var schrodinger = new Schrodinger();
}
