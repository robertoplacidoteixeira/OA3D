class RptMath extends RptCommon {
	
	beforePrepare() {
		$g.math.pi = Math.PI;
		$g.math.eulerGamma = .5772156649015329;
		this.decimals = 0;
	...
	afterCreate() {
		...
		this.D = this.this.diff;
		this.real = re;
		this.imag = im;
	}

	getConstant(name) {
		return this.constants[name] * this.precisionScale / this.constants.precisionScale;
	}
	...
	setPrecisionScale(n) {
		...
		this.onePi = this.getConstant('pi')
	...
	jacobiTheta(n,x,q,tolerance=1e-10) {
		...
		if (this.isComplex(x) || this.isComplex(q)) {
			...
			if (Math.abs(x.im) > Math.abs(piTau.im) || Math.abs(x.re) > Math.PI) {
				...
				return
					(n === 1) ? this.mul((-1)**(p+pt),qFactor,eFactor,this.chop(this.jacobiTheta(n,x,q),tolerance)) :
					(n === 2) ? this.mul((-1)**p,qFactor,eFactor,this.chop(this.jacobiTheta(n,x,q),tolerance)) :
					(n === 3) ? this.mul(qFactor,eFactor,this.chop(this.jacobiTheta(n,x,q),tolerance)) :
					(n === 4) ? this.mul((-1)**pt,qFactor,eFactor,this.chop(this.jacobiTheta(n,x,q),tolerance));
				}
			}
			...
			switch(n) {
				case 1:
					var s = iterator(0,function(i){t.mul((-1)**i,t.pow(q,i*i+i),t.sin(t.mul_(2*i+1,x)))};
	...
	beta(x,y,z,w) {
		return (arguments.length === 4) ?
			...
			(arguments.length === 3) ? 
				this.mul(this.pow(x,y),this.inv(y),hypergeometric2F1(y,this.sub(1,z),this.add(y,1),x)) :
	...
	expIntegralEi(x,tolerance=1e-10) {
		...
		if (this.isComplex(x)) {
			...
			if (useArbitrary) {
				...
				s = this.add(s,this.getConstant('eulerGamma'),this.ln(y));
				s = this.arbitrary(s);
			} else {
				var s = this.complex(0);
				var p = this.complex(1);
				...
				s = this.add(s,eulerGamma,this.log(x));
			}
		...
		if (this.isComplex(a) || this.isComplex(b) || this.isComplex(c) || this.isComplex(x)) {
			...
			switch(index) {
				case 0:
					break;
				case 1:
					return this.mul(this.pow(this.sub(1,x),this.neg(a)),hypergeometric2F1(a,this.sub(c,b),c,this.div(x,this.sub(x,1))));
				case 2:
					if (this.isInteger(this.sub(c,this.add(a,b))) || this.isNegativeIntegerOrZero(this.sub(c,a)))
						return this.complexAverage(a => hypergeometric2F1(a,b,c,x),a);
					if (this.isNegativeIntegerOrZero(this.sub(c,b)))
						return this.complexAverage(b => hypergeometric2F1(a,b,c,x),b);
					var t1 = this.mul(
							this.gamma(c),this.gamma(this.sub(c,this.add(a,b))),
							this.inv(this.gamma(this.sub(c,a))),this.inv(this.gamma(this.sub(c,b))),
							hypergeometric2F1(a,b,this.add(a,b,this.neg(c),1),this.sub(1,x)));
					var t2 = this.mul(
							...
							hypergeometric2F1(this.sub(c,a),this.sub(c,b),this.add(c,this.neg(a),this.neg(b),1),this.sub(1,x)));
					return this.add(t1,t2);
				case 3:
					...
					var t1 = this.mul(
						...
						hypergeometric2F1(a,this.add(1,this.neg(c),a),this.add(1,this.neg(b),a),this.inv(x)));
					var t2 = this.mul(
						...
						hypergeometric2F1(b,this.add(1,this.neg(c),b),this.add(1,this.neg(a),b),this.inv(x)));
					return this.add(t1,t2);
				case 4:
					if (this.isInteger(this.sub(a,b)) || this.isNegativeIntegerOrZero(this.sub(c,a)))
						return this.complexAverage(a => hypergeometric2F1(a,b,c,x),a);
					...
					var t1 = this.mul(
							...
							hypergeometric2F1(a,this.sub(c,b),this.add(a,this.neg(b),1),this.inv(this.sub(1,x))));
					var t2 = this.mul(
						...
						hypergeometric2F1(b,this.sub(c,a),this.add(b,this.neg(a),1),this.inv(this.sub(1,x))));
					return this.add(t1,t2);
				case 5:
					if (this.isInteger(this.sub(c,this.add(a,b))) || this.isNegativeIntegerOrZero(this.sub(c,a)))
						return this.complexAverage(a => hypergeometric2F1(a,b,c,x),a);
					...
					var t1 = this.mul(
						...
						hypergeometric2F1(a,this.add(a,this.neg(c),1),this.add(a,b,this.neg(c),1),this.sub(1,this.inv(x))));
					var t2 = this.mul(
						this.gamma(c),this.gamma(this.sub(this.add(a,b),c)),this.inv(this.gamma(a)),this.inv(this.gamma(b)),
						this.pow(this.sub(1,x),this.sub(c,this.add(a,b))),this.pow(x,this.sub(a,c)),
						hypergeometric2F1(this.sub(c,a),this.sub(1,a),this.add(c,this.neg(a),this.neg(b),1),this.sub(1,this.inv(x))));
					return this.add(t1,t2);
			}
			...
			if (x === -1) return hypergeometric2F1(a,b,c,this.complex(x)).re;
			if (x === 1)
			...
			if (x > 1) return hypergeometric2F1(a,b,c,this.complex(x));
			var s = 1;
			var p = 1;
			var i = 1;
			while (Math.abs(p) > tolerance) {
	...
	ln(x) {
		...
		function arbitraryAGM(x,y) {
			var t,u,this.arb2 = this.arbitrary(2);
			if (this.isComplex(x)) {
				var maxIter = 10,i = 0;
		...
		if (this.isArbitrary(x)) {
			if (!this.isComplex(x)) {
				if (x < 0n) return { re: this.ln(-x),im: this.getConstant('this.pi') };
	...
	legendreP(l,m,x,renormalized=false) {
		...
		return (this.isPositiveInteger(m)) ?
			this.mul(
				...
				hypergeometric2F1(this.neg(l),this.add(l,1),this.add(m,1),this.div(this.sub(1,x),2))) :
			this.mul(
				this.inv(this.gamma(this.sub(1,m))),
				this.pow(this.add(1,x),this.div(m,2)),this.pow(this.sub(1,x),this.div(m,-2)),
				hypergeometric2F1(this.neg(l),this.add(l,1),this.sub(1,m),this.div(this.sub(1,x),2)));
	}
	...
	doubleLambert(n,x,y,tolerance=1e-10) {
		if (this.isZero(y)) return lambertW(n,x);
		if (this.isZero(x)) return this.neg(lambertW(-n,this.neg(y)));
		...
		function start(n,x,y,tolerance=1e-3) {
			...
			w1 = lambertW(k,x,tolerance);
			w2 = this.neg(lambertW(-k,this.neg(y),tolerance));
	...
	function sin(x) {
		if (this.isArbitrary(x)) {
			...
			if (x < -this.halfPi) return sin(-this.onePi - x);
			var s = x;
			var p = x;
			var i = this.arb2;
			while (p !== 0n) {
	...
	tan(x) {
		return this.isComplex(x) ? this.div(sin(x),this.cos(x)) : Math.tan(x);
	 }
	...
	function arccoth(x) {
		...
		return Math.abs(x) > 1 ? Math.atanh(1/x) : arccoth(this.complex(x));
	}

	function arc.sech(x) {
		if (this.isComplex(x)) {
			if (x.re === 0 && x.im === 0) throw Error('Indeterminate arc.sech value');
			// adjust for branch cut along negative axis
			if (x.im === 0) x.im = -Number.MIN_VALUE;
			return this.arccosh(this.div(1,x));
		}
		return (x > 0 && x < 1) ? Math.acosh(1/x) : arc.sech(this.complex(x));
	}
	...
	zeta(x,tolerance=1e-10) {
		...
		if (x < 0 || x.re < 0)
			return this.mul(this.pow(2,x),this.pow(this.pi,this.sub(x,1)),sin(this.mul(this.pi/2,x)),this.gamma(this.sub(1,x)),this.zeta(this.sub(1,x)));
	...
	harmonic(n) {
		...
		if (n > 1e3) return this.log(n) + eulerGamma + 1/2/n - 1/12/n**2;
	...
	findExtremum(f,point,options={}) {
		...
		var maxIter = 1e4;
		var this.gamma = .01 * sign;
		var grad,step,test;
		for (var i = 0 ; i < maxIter ; i++) {
			grad = this.gradient(f,point);
			test = true;
			for (var j = 0 ; j < point.length ; j++) {
				step = this.gamma * grad[j];
				point[j] += step;
	...
	polynomial(x,coefficients,derivative=false) {
		...
		return derivative ? {this.polynomial: p,derivative: q} : p;
	}
