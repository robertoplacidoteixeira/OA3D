class RptMath extends RptCommonAncestor {
	...
	setPrecisionScale(n) {
		...
		this.ln10 = this.ln(this.arbitrary(10));
	}
	...
	exp(x) {
		if (this.isArbitrary(x)) {
			...
			var m = Math.trunc(this.arbitrary(this.div(x,this.ln10)));
			x = x - this.mul(this.arbitrary(m),this.ln10);
	...
	ln(x) {
		if (this.isArbitrary(x)) {
			...
			var t2 = this.arbitraryTheta2(x);
			var t3 = this.arbitraryTheta3(x);
			var result = this.div(this.onePi,this.mul(this.arbitrary(4),this.arbitraryAGM(this.mul(t2,t2),this.mul(t3,t3))));
